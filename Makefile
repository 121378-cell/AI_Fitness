.PHONY: migrate validate quality pipeline pipeline-json quality-json test-services test-coverage ci-check
...
ci-check:
	python -m unittest -v tests.test_services tests.test_config_settings tests.test_database_migrations tests.test_integration_pipeline
	python tools_check_coverage.py --fail-under 45
